...
from timesformer.models.transnpl import TransNPL
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0.1, hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=8, attention_type='divided_space_time', npl_loc=4, dropout=0.):
        super().__init__()
        ...
        self.npl_loc = npl_loc
        self.transnpl = TransNPL(config)
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            '''
            if i == self.npl_loc:
                x = self.transnpl(x, self.pos_embed, B, T, W)
            '''
            x = blk(x, B, T, W)
        x = self.transnpl(x, self.pos_embed, B, T, W)
